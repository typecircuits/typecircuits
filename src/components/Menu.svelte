        ...
        <div bind:this={wrapper} use:portal class="fixed z-100" role="menu">
            {#if contentVisible}
                ...
                <div
                    ...
                    onclick={dismiss}
                    class="flex min-w-[150px] flex-col items-stretch justify-start gap-[4px] overflow-auto rounded-[10px] border-[1.5px] border-black/5 bg-white px-[4px] py-[4px] text-sm shadow-lg shadow-black/5"
                >
                    {@render items()}
                </div>
            {/if}
